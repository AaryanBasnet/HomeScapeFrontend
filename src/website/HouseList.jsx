...
const HouseList = ({limit}) => {
  ...
  const displayedHouses = limit ? houses.slice(0, limit) : houses;//if limit is true, display the first two houses
      ...
      <div className="container mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          {displayedHouses.map((house, index) => (
            <Link to={`/property/${house.id}`} key={index} className="block">
              <House house={house} />
            </Link>
          ))}
        </div>
